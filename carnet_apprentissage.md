...
| Projet | Harnais | Loop |
|---|---|---|
| Projet 2 — Rangement fichiers | Outils : scan FS (pull) + tool-calling forcé Anthropic · Règles : jamais lire sans permission, renommer seul mais jamais déplacer sans clic · Mission : ranger ~/Downloads | Bouton "Scanner" → Lire (sur clic) → Renommer automatique (tool-calling) → Proposer déplacement → Attendre confirmation → Ranger → Logger *(anticipé ici avec `inotify` avant construction, session 1 — corrigé en session 11 : mode pull retenu, pas de surveillance temps réel construite pour l'instant)* |
...
## Procédure — Audit de fin de brique/projet (Claude Code ↔ Claude)
*Réutilisable, pas spécifique à un projet précis — à appliquer à la fin de chaque brique significative. Introduite en session 11 (Projet 2).*

1. **Claude Code produit une explication du code construit, avec le vrai code, pas un résumé en prose.** Demander explicitement les extraits exacts (fonctions, logique réelle), pas une paraphrase narrative — sinon on reproduit l'erreur déjà identifiée dans ce projet (rapport technique, session 7) : faire confiance à un résumé d'agent sans vérifier le code réel derrière.
2. **Cette explication (avec le vrai code) est transmise à Claude (chat), qui imagine des scénarios de défaillance.** Valeur ajoutée spécifique de cette étape : Claude (chat) porte tout l'historique du projet — cahier des charges, décisions actées, règles explicites (ex : contrainte same-directory sur `renommer_fichier`) — et peut comparer l'implémentation réelle à l'intention documentée, pas seulement chercher des failles génériques. Angle mort différent de ce qu'un audit générique type Skill `security-review` peut voir en partant du seul code, sans connaître les règles spécifiquement décidées pour ce projet.
3. **Les scénarios générés sont repassés à Claude Code, qui écrit et exécute de vrais tests automatisés** pour chacun — pas une simple relecture, une vérification reproductible (même principe que la section 6c du brief Projet 2 : tests > confiance).

**Point de vigilance** : cette boucle peut recouper partiellement ce qu'un Skill `security-review` automatique fait déjà tout seul dans Claude Code (voir Recueil d'exemples commentés, ligne Projet 2/session 11) — complémentaire, pas dupliqué à l'identique : la boucle manuelle apporte spécifiquement la comparaison à l'intention documentée du projet, ce que l'audit automatique seul n'a pas forcément.
...
| `~/mcp-servers/Gmail-MCP-Server/` (serveur MCP Gmail) + `~/.gmail-mcp/` (credentials OAuth) | Installé hors du repo Git (comme Calendar) ; les credentials OAuth authentifiés sont stockés séparément, dans `~/.gmail-mcp/`, pas dans le dossier du serveur. **Contient aussi un correctif local fait à la main** (depuis la session 10) dans `src/tools.ts` : l'outil `download_attachment` est verrouillé derrière le scope `gmail.full` (jamais accordé) au lieu de `gmail.readonly`/`gmail.modify`, pour empêcher tout téléchargement de pièce jointe même si le jeton le permettrait techniquement | Réinstaller (`git clone` + `npm install`, le build tourne automatiquement via le script `prepare`), **réappliquer manuellement le correctif `download_attachment` → `gmail.full` dans `src/tools.ts`** puis `npm run build` (le code patché n'est sauvegardé nulle part ailleurs, comme le correctif Calendar ci-dessus), copier un `gcp-oauth.keys.json` valide (même projet GCP que Calendar) et relancer `node dist/index.js auth --scopes=gmail.readonly` pour ré-authentifier (scope minimal depuis la session 10 — ne jamais redemander `gmail.modify`, retiré exprès des scopes déclarés sur l'écran de consentement) |
...
| Identité visuelle du dashboard (session 10) | Système de couleur **par zone** ("module de contrôle") : chaque zone garde sa propre teinte même au repos (rose notes, vert tâches, bleu mail, violet calendrier, or coûts) ; chat et suggestions partagent la même famille corail — la "voix de Claude" du dashboard | Le skill `frontend-design` signale explicitement que fond crème + accent terracotta + serif (la direction initiale de ce dashboard, session 2) est l'un des 3 "looks par défaut" que produit une IA sans direction précise — remplacé par un système dérivé du sens de chaque zone plutôt que d'une palette générique. **Écarté** : garder l'accent unique calqué sur claude.ai — cohérent avec la marque Claude, mais l'utilisateur a explicitement demandé une identité propre au dashboard. |
| Typographie du dashboard (session 10) | **Fraunces** (serif variable, chaleureux) remplace `Source Serif 4` pour tout le contenu lu (briefing, mails, chat) ; `Inter` conservé pour l'interface | Correspond à la direction "moderne, chaleureuse, vivante" choisie par l'utilisateur — l'axe optical-size de Fraunces donne un rendu plus caractériel que `Source Serif 4`, plus neutre. **Écarté** : garder `Source Serif 4` — fonctionnel mais sans personnalité propre à ce projet. |
| Animation d'ouverture des zones (session 10) | Halo coloré + léger effet de zoom à l'ouverture, zones voisines qui s'estompent — **entièrement en CSS** (sélecteur `:has()`), zéro changement dans `script.js` | Répond à la demande "animations plus expressives" en réutilisant la classe `.ouvert` déjà posée par `script.js` pour d'autres raisons — `:has()` permet de réagir à cet état côté CSS sans dupliquer la logique en JS. Supporté par Chrome, seul navigateur utilisé dans ce projet. **Écarté** : ajouter une classe dédiée en JS sur chaque zone pour piloter l'estompement des voisines — aurait dupliqué un état déjà disponible sans bénéfice. |
| Badges de compteur sur les zones fermées (proposé par l'utilisateur, non implémenté — session 10) | **Pas construit** malgré la demande initiale | Remplir un vrai badge (mails urgents, deadlines) demanderait d'appeler `trier_emails_urgents()`/`construire_briefing()` — tous deux payants côté API Anthropic — automatiquement à chaque chargement de page. Contredit la règle "Claude n'agit jamais sans demande" et générerait des coûts non sollicités. **Écarté** : remplir silencieusement un badge au chargement — reporté à une session où les données nécessaires (tâches notamment) seront déjà chargées pour d'autres raisons. |
| Suppression de la capacité d'envoi de mail (session 10) | **Triple garantie cumulée** : jeton OAuth repassé en scope `gmail.readonly` (Google refuse la capacité elle-même) + `gmail.modify` retiré des scopes déclarés sur l'écran de consentement (empêche même une ré-authentification accidentelle future en scope large) + suppression complète de la route `/api/gmail/<id>/send` et des fonctions `envoyer_email()`/`extraire_adresse()` côté code | Demande explicite de l'utilisateur : "aucune once de doute" que le dashboard ne puisse jamais envoyer de mail. Reprend et durcit à son maximum la doctrine déjà actée en session 6 (le scope OAuth est une garantie appliquée par Google, plus forte qu'un bouton UI) : même un bug de code ne pourrait plus envoyer, le verbe `send_email` n'existe plus du tout côté serveur MCP avec ce scope. **Écarté** : se contenter de retirer le bouton "Envoyer" côté interface — la session 7 avait déjà démontré qu'un bouton UI seul n'est pas un verrou technique. |
| Remplacement du flux d'envoi par brouillon + lien Gmail (session 10) | Bouton "Générer un brouillon" (inchangé dans le principe) → texte éditable + bouton "Copier" (presse-papiers) + bouton "Ouvrir dans Gmail ↗" qui ouvre le vrai fil (`https://mail.google.com/mail/u/0/#all/<id>`) dans un nouvel onglet | L'utilisateur clique lui-même "Répondre" côté Gmail, qui gère alors nativement le sujet `Re:`, la citation et le threading — sans que le dashboard n'ait besoin d'aucune capacité d'envoi. **Écarté** : lien "compose" avec destinataire pré-rempli — envisagé, jugé moins fiable pour préserver le bon fil de discussion Gmail (choix explicite de l'utilisateur face à ce compromis). |
| Verrouillage du téléchargement de pièces jointes (session 10) | Patch du code source du serveur MCP Gmail tiers (`~/mcp-servers/Gmail-MCP-Server/src/tools.ts`) : l'outil `download_attachment` requiert désormais `gmail.full` (jamais accordé) au lieu de `gmail.readonly`/`gmail.modify`, puis recompilation (`npm run build`) | Google ne propose aucun scope OAuth séparant "lire le contenu d'un mail" de "télécharger une pièce jointe" (les deux sont couverts par `gmail.readonly`) — impossible d'obtenir une garantie au niveau du jeton comme pour l'envoi. Le verrou est donc posé à la couche du serveur MCP lui-même, en réutilisant un pattern déjà présent dans ce serveur pour `delete_email`/`batch_delete_emails` (verrouillés derrière `gmail.full` pour rester "least-privilege" par défaut). **Écarté** : ne rien faire (le code du dashboard n'appelait déjà jamais `download_attachment`) — jugé insuffisant par l'utilisateur, qui voulait une garantie plus dure qu'"on ne l'utilise juste jamais". |
| Tool-calling pour le renommage/rangement de fichiers (Projet 2, session 11) | **Tool-calling réel** (`tools=` avec deux fonctions, `renommer_fichier` + `proposer_deplacement`), forcé sur l'API Anthropic quel que soit le provider actif ailleurs dans le dashboard — première fois dans tout le projet | Rupture assumée avec le principe du Projet 1 (aucun `tools=` nulle part, vérifié par audit session 7). Deux tests envisagés puis écartés en cours de réflexion : réponse texte simple relue par l'utilisateur avant toute action (le renommage aurait alors demandé une confirmation par fichier, jugé trop de friction pour une action à faible conséquence) ; Ollama/modèle local (gratuit, cohérent avec la philosophie du projet, mais jugé pas assez fiable pour une action qui s'exécute sans confirmation humaine — la fiabilité prime ici sur le coût). |
| Split renommer (auto) / déplacer (confirmé) (Projet 2, session 11) | `renommer_fichier` s'exécute **automatiquement**, sans confirmation, dès que la lecture du fichier a été autorisée ; `proposer_deplacement` reste **toujours soumis à confirmation** explicite dans le dashboard | Amendement conscient à la règle générale du projet "Claude suggère, n'applique jamais sans confirmation" (cahier des charges) : un renommage seul, sans changement de dossier, est jugé à conséquence faible et réversible (fichier au même endroit, ancien nom loggé) — contrainte non négociable en contrepartie : `renommer()` vérifie que la destination reste dans le même dossier, testé explicitement (`test_renommer_ne_peut_pas_changer_de_dossier`). Le déplacement, à conséquence plus haute (changement de zone), garde lui la confirmation. |
| Mode de détection des nouveaux fichiers (Projet 2, session 11) | **Pull** : bouton "Scanner" dans le dashboard, comparaison à `fichiers_traites.json` (même principe que `mails_notifies.json`) | Corrige une hypothèse antérieure du carnet (tableau "Exemples concrets", plus haut) qui anticipait un déclenchement par `inotify` — écarté au moment de construire : aucun processus du projet ne tourne encore en continu (`gmail_watcher.py`, MCP Calendar, tous manuels), ajouter un premier watchdog fiable aurait été un chantier d'infra à part entière plutôt qu'un détail de cette session. Reporté explicitement à après l'automatisation `systemd` du serveur Flask lui-même. |
| Skill Anthropic dédié au Projet 2 (session 11) | `~/.claude/skills/rangement-fichiers/SKILL.md`, consulté par **Claude Code** (l'outil de dev) en session interactive | Contrairement au Projet 1 (pas de tool-calling, donc pas d'usage pertinent pour un vrai Skill), l'introduction du tool-calling ici justifie une mémoire procédurale — mais son usage réel est pour l'assistant de développement, pas pour `fichiers_manager.py` à l'exécution (qui n'a pas de boucle agentique consultant de Skill). Les règles de nommage/permissions utilisées par le script lui-même restent du code Python classique (constantes, regex), pas un Skill au sens strict. |
| Vérification du code de `fichiers_manager.py` (session 11) | **Revue adversariale par un agent** (4 scénarios d'attaque ciblés : traversée de chemin, lien symbolique, déguisement renommage→déplacement, valeur d'outil forgée) + suite de tests automatisés (60 tests, `test_fichiers_manager.py`) épinglant les bugs trouvés | Deux vrais bugs trouvés que la relecture manuelle n'avait pas vus : un octet nul (`\x00`) passait la regex de validation et provoquait un crash non rattrapé côté OS ; renommer un fichier vers son propre nom (cas explicitement demandé au modèle, "toujours proposer un nom même si déjà bon") était traité comme une collision et refusé à chaque fois. **Écarté** : se fier à la seule relecture de code (`grep`) déjà faite — jugée insuffisante après l'incident du 01/08/2026 (suppression d'un fichier de test sans autorisation), un agent qui approuve n'étant pas une preuve. |
| Audit de sécurité de `fichiers_manager.py`, observé en direct dans Claude Code (session 11) | Skill `security-review`, qui délègue lui-même à un sous-agent (mission "Security review step 1 — identify vulnerabilities", ~12 appels d'outils, ~5 min, ~58,7k tokens) | Illustration concrète, vue en direct, de deux mécanismes du glossaire (Skill au sens Anthropic, sous-agent) : un Skill reconnu et chargé automatiquement par pertinence (pas parce qu'une section du cahier des charges le demande), qui exécute via un sous-agent à contexte isolé plutôt que dans le fil de la conversation principale — cohérent avec la leçon "une revue indépendante doit repartir de zéro, pas vérifier la conformité au plan" déjà tirée à la session 7. |
| Désambiguïsation de dossiers similaires lors d'un rangement global (Projet 2, pas encore construit — noté en session 11 pour plus tard) | Tool-calling avec un outil `lister_fichiers_dans_dossier()`, plutôt qu'un aller-retour texte+script classique | Cas où le nombre et la nature des étapes ne peuvent pas être fixés d'avance dans du code Python : pour départager deux dossiers au nom proche (ex : `Finance` vs `Finance-Internationale`), il faut parfois regarder leur contenu — mais seulement pour les cas ambigus, dont le nombre dépend de ce qui est découvert en cours de route. Le pattern "texte+script" obligerait à tout précharger dans le prompt par avance (gaspillage) ; le tool-calling permet à Claude de n'appeler l'outil que quand c'est réellement nécessaire, autant de fois que nécessaire — différence structurante avec `renommer_fichier`/`proposer_deplacement` (un seul appel prévisible par fichier). |
...
| **Agent IA** | Programme qui ne se contente pas de répondre une fois, mais **observe → réfléchit → agit → observe le résultat → recommence**, de façon autonome sur plusieurs étapes. Composé de deux parties : le **harnais** (les outils autorisés, les règles à respecter, la mission) et la **loop** (la boucle d'exécution elle-même). Toujours pas construit dans ce projet au sens strict (boucle autonome, sans clic) — `briefing.py` (étape 4) et maintenant `fichiers_manager.py` (Projet 2, session 11) s'en approchent par leur chaînage (plusieurs étapes qui se nourrissent l'une l'autre), mais restent des chaînes courtes déclenchées par des clics explicites successifs, pas un agent autonome multi-tours. La surveillance 24/7 (watchdog) qui rapprocherait vraiment le Projet 2 d'un agent reste volontairement reportée (voir session 11). |
| **Skill** | Au sens large du carnet : règle ou procédure fixe, apprise une fois, réutilisable — pas d'autonomie ni de boucle, contrairement à un agent. Au sens strict Anthropic (dossier avec un `SKILL.md`) : réellement construit pour la première fois en session 11 (`~/.claude/skills/rangement-fichiers/SKILL.md`), mais comme mémoire procédurale pour **Claude Code lui-même** (l'outil de dev, qui le consulte en session interactive) — pas comme mécanisme consulté à l'exécution par `fichiers_manager.py`, qui n'a pas de boucle agentique et n'a donc aucun Skill à lire. Les deux usages du mot ne se recouvrent pas forcément. |
...
| **Scopes déclarés vs scope accordé (OAuth)** | Deux couches distinctes, souvent confondues. Les **scopes déclarés** sont configurés une fois en amont sur l'écran de consentement du projet (Cloud Console) : la liste de tout ce que l'appli a *le droit de demander un jour*. Le **scope accordé** est ce qu'un compte précis a *réellement autorisé* lors d'une authentification donnée, stocké dans le jeton. Demander un scope jamais déclaré bloque toute la procédure (écran "Accès bloqué" générique côté Google) — piège rencontré en session 10 : facile à confondre avec un problème de liste de testeurs, qui produit exactement le même écran d'erreur pour une cause différente. |
| **Testeur OAuth (Testing / Publishing status)** | Un projet Google Cloud en statut "Testing" (non vérifié par Google) limite l'usage de l'appli à une liste de comptes explicitement ajoutés ("testeurs"), jusqu'à 100. Être testeur ne donne **aucun accès automatique** : ça rend seulement le compte éligible à passer l'écran de consentement *s'il le fait lui-même* et clique "Autoriser" — sans cette étape, rien n'est accordé, vérifiable concrètement en regardant si le fichier de jeton local a changé. |
| **Écran "Accès bloqué" / `access_denied` (Google)** | Message générique que Google affiche dans plusieurs situations différentes pour une appli en mode Testing : compte non testeur, scope non déclaré sur l'écran de consentement, ou mauvais compte actif dans le navigateur (plusieurs comptes Google connectés en même temps). Le message ne précise pas laquelle des causes s'applique — à diagnostiquer une par une plutôt que de supposer. |
| **Tool-calling / function-calling** | Un modèle IA ne peut, par défaut, que produire du texte — jamais agir directement sur le monde réel. Le paramètre `tools=` de l'appel API lui donne une liste de fonctions nommées et décrites ; le modèle peut alors répondre "j'appelle la fonction X avec ces paramètres", mais c'est toujours le code appelant qui décide d'exécuter réellement cette demande. Absent partout ailleurs dans ce projet (Projet 1, `ia_provider.py` — vérifié par `grep tools=`, zéro résultat) ; utilisé pour la première fois en session 11 (Projet 2, `proposer_organisation_fichier()`), rupture assumée et documentée, pas un oubli de cohérence. |
| **Permission logicielle vs permission OS** | Une permission OS (`chmod`, droits Unix) est vérifiée par le noyau avant même que le programme s'exécute — barrière externe au code. Une permission logicielle est une règle écrite dans le code lui-même (un `if`) : elle ne protège que si le code l'implémente correctement partout, sans oubli. Le Projet 2 (`fichiers_manager.py`) tourne avec les droits Linux normaux de l'utilisateur (accès déjà techniquement possible à tout `$HOME`) — sa matrice de permissions par zone est entièrement de ce second type. |
| **Canonicalisation de chemin** | Résoudre un chemin de fichier vers sa forme réelle absolue (`Path.resolve()` / `os.path.realpath()`), en particulier en suivant les liens symboliques — nécessaire avant de vérifier qu'un chemin appartient bien à un dossier autorisé, car un chemin "en apparence correct" peut pointer ailleurs. Implémenté dans `fichiers_manager.resoudre_chemin()`, seule porte d'entrée vers le disque pour ce module. |
| **Lien symbolique (symlink)** | Un fichier qui n'est qu'un raccourci pointant vers un autre emplacement réel du disque. Un programme qui agit sur "le fichier" sans résoudre le lien risque d'agir en réalité sur la cible du lien, potentiellement hors du périmètre prévu — cas testé explicitement en session 11 (`test_fichiers_manager.py`). |
| **Fonction centralisée (single entry point)** | Point de passage unique obligatoire pour toutes les opérations d'un même type dans un projet (ex : `resoudre_chemin()` pour tout accès disque du Projet 2, ou `generer_reponse()` pour tous les appels IA). Réduit la surface de code à auditer et à tester à un seul endroit plutôt que dispersée dans tout le projet. |
| **Confiance vs vérification (méthode)** | Faire confiance à du code écrit par une IA repose sur une expectative non testable ("un agent l'a relu, donc c'est bon"). Vérifier repose sur des méthodes reproductibles : tests automatisés qui tentent explicitement de casser une règle, audit par recherche exhaustive (`grep`) dans le code réel, revue croisée par un second modèle. Concrétisé en session 11 : une revue adversariale a trouvé deux vrais bugs (caractère nul non rattrapé, renommage vers le même nom toujours refusé) qu'une simple relecture n'avait pas vus — épinglés ensuite en tests de non-régression. |
| **Harnais d'agent vs API brute (pour les Skills)** | Un harnais d'agent (Claude Code, Claude.ai, Claude Desktop) enveloppe l'appel au modèle avec des fonctionnalités en plus, dont la consultation automatique de Skills : c'est le harnais qui scanne les dossiers Skills et injecte lui-même leur contenu dans le contexte avant l'appel API. L'API brute (`api.anthropic.com/v1/messages`, celle qu'appelle `generer_reponse()`) n'a pas cette fonctionnalité, même avec `tools=` configuré — `tools=` et la consultation de Skill sont deux mécanismes indépendants. Un script qui appelle l'API brute doit lui-même insérer les règles dans le prompt, il ne peut pas compter sur le modèle pour "aller lire" un Skill tout seul — c'est pour ça que `fichiers_manager.py`/`ia_provider.py` n'utilisent jamais le Skill `rangement-fichiers`, seulement Claude Code en session interactive. |
| **Contrôle préventif vs contrôle détectif** | Un contrôle préventif empêche une action non conforme de se produire (ex : `renommer()` qui rejette tout chemin changeant de dossier). Un contrôle détectif n'empêche rien mais permet de constater après coup qu'une action a eu lieu, avec quoi et quand (ex : le journal d'activité, `activity_log.py`). Une action automatique sans confirmation humaine (comme `renommer_fichier`, session 11) ne garde que le second comme filet — d'où l'importance de logger sans exception dès qu'une confirmation humaine est retirée. |
| **`CLAUDE.md` vs `SKILL.md` (Claude Code)** | Deux fichiers distincts, rôles différents. `CLAUDE.md` : un seul par projet, toujours chargé automatiquement à chaque session — pour tout ce qui est "dans ce projet, vrai en permanence" (ex : une matrice de permissions pertinente pour toute tâche touchant un fichier). `SKILL.md` : une capacité réutilisable, chargée seulement quand la tâche en cours y correspond, portable entre plusieurs projets. Séparer les deux évite de tout recharger en permanence (contexte permanent = léger) tout en gardant les procédures ponctuelles disponibles sans les charger à chaque fois (économie de tokens). |
| **Skills API (bêta, API brute)** | Contrairement à une affirmation initiale trop catégorique lors de la conception du Projet 2, l'API brute a bien un mécanisme Skills — paramètre `container={"skills": [...]}` + outil d'exécution de code sandboxé, fonctionnalité bêta. Nuance : demande de référencer explicitement un `skill_id` à chaque appel (pas de découverte automatique par pertinence comme dans Claude Code), et cible surtout la génération de fichiers via code sandboxé — disproportionné pour une règle de nommage simple comme celle du Projet 2, où insérer les règles dans le prompt reste la bonne approche. |
| **Sous-agent (subagent)** | Une instance Claude séparée, lancée par une session Claude Code (ou un Skill) pour une mission précise et bornée, avec son propre contexte (fenêtre fraîche, n'hérite pas de tout l'historique de la conversation principale) et son propre budget d'appels d'outils/tokens. Tourne en autonomie puis renvoie son résultat à la session appelante. Avantage concret pour un audit de sécurité : un sous-agent qui repart de zéro (sans le contexte de conception complet) est moins susceptible de se contenter de vérifier "est-ce conforme à ce qui était prévu" — il doit retrouver ou contourner les protections par lui-même, plus proche d'une revue réellement indépendante. Observé en direct en session 11 via le Skill `security-review`. |
...
### Session 10 — Refonte visuelle du dashboard + verrouillage sécurité Gmail (suppression envoi + pièces jointes) ✅ Terminée
**Date :** 19/07/2026
**Durée :** —
**Étape du projet :** Suivi post-étape 4 — refonte visuelle et durcissement sécurité Gmail, aucune nouvelle fonctionnalité métier

#### Ce qu'on a fait

**Partie 1 — Refonte visuelle du dashboard (skill `frontend-design`)**
- Skill `frontend-design` chargé avant toute retouche visuelle (consigne du skill lui-même : calibrer l'investissement design avant d'écrire du CSS)
- Questions de cadrage posées à l'utilisateur avant de coder : portée sur le layout figé, lien ou non avec l'identité visuelle claude.ai, ambiance recherchée, niveau d'animation — réponses obtenues : proportions ajustables mais grille figée conservée, identité propre (pas un calque de claude.ai), ambiance "moderne, chaleureuse, vivante", animations "plus expressives"
- Nouveau système de couleur **par zone** ("module de contrôle") : chaque zone garde sa propre teinte même au repos, pas seulement au clic — rose (notes), vert (tâches), bleu (mail), violet (calendrier), or (coûts) ; chat et suggestions partagent une même famille corail, la "voix de Claude" du dashboard
- Typographie : `Fraunces` (serif variable, chaleureux) remplace `Source Serif 4` pour tout le contenu lu (briefing, mails, chat) ; `Inter` conservé pour l'interface
- Animation signature à l'ouverture d'une zone : halo coloré + léger effet de zoom ("bloom"), zones voisines qui s'estompent légèrement — entièrement en CSS (sélecteur `:has()`), aucune modification de `script.js`
- Vérification visuelle réelle avant de considérer le travail terminé (conforme à la consigne du projet) : serveur Flask lancé, Playwright (déjà en cache localement, pas de téléchargement nécessaire) utilisé pour piloter un Chromium headless, captures d'écran au repos et zones ouvertes, lues et jugées avant de continuer
- Itération après la première capture : fond des cartes légèrement teinté (`color-mix`) ajouté après un premier rendu jugé trop blanc/vide au repos malgré les pastilles colorées

**Partie 2 — Suppression de la capacité d'envoi de mail + verrouillage des pièces jointes**
- Demande explicite de l'utilisateur : supprimer *totalement* la possibilité d'envoyer un mail depuis le dashboard, remplacer par un brouillon + lien vers Gmail pour copier-coller manuellement — "aucune once de doute" qu'aucun mail ne puisse partir depuis le dashboard
- Suppression côté code : fonctions `envoyer_email()`/`extraire_adresse()` (`gmail_mcp.py`), route `/api/gmail/<id>/send` (`server.py`), bouton "Envoyer" (`script.js`) — remplacés par "Générer un brouillon" (inchangé) + bouton "Copier" (presse-papiers) + bouton "Ouvrir dans Gmail ↗" (ouvre le vrai fil dans un nouvel onglet, l'utilisateur clique "Répondre" lui-même côté Gmail)
- Suppression côté jeton OAuth : ré-authentification du serveur MCP Gmail avec `--scopes=gmail.readonly` (au lieu de `gmail.modify`) — l'outil `send_email` n'existe plus du tout côté serveur avec ce scope
- Verrouillage des pièces jointes : patch du code source tiers (`~/mcp-servers/Gmail-MCP-Server/src/tools.ts`) — `download_attachment` requiert désormais `gmail.full` (jamais accordé) au lieu de `gmail.readonly`/`gmail.modify`, recompilé (`npm run build`), vérifié dans le fichier compilé réellement exécuté (`dist/tools.js`)
- `gmail.modify` retiré de la liste des scopes déclarés sur l'écran de consentement Google Cloud (section Data Access) — empêche même une future ré-authentification accidentelle avec ce scope large
- `cahier_des_charges.md` mis à jour (v4.1 → v4.2) : règle critique ajoutée, table de validation des actions, section Fonctionnalités Gmail, contraintes globales

**Dépannage rencontré pendant la ré-authentification**
- Écran "Accès bloqué" de Google (403 `access_denied`) au premier essai : diagnostic = scope `gmail.readonly` jamais déclaré sur l'écran de consentement (seuls `gmail.modify`/`gmail.settings.basic` l'étaient depuis la session 6) → ajouté manuellement par l'utilisateur dans Cloud Console
- `EADDRINUSE` sur le port 3000 au deuxième essai : un processus Node totalement indépendant (`node build/index.js --transport http --port 3000`, lancé le matin même, sans rapport avec ce projet) occupait déjà le port par défaut du callback OAuth — identifié via `lsof`/`ss` avant de toucher à quoi que ce soit, résolu en pointant l'auth vers le port 3001 (URL de callback personnalisée passée en argument, acceptée sans reconfiguration côté Google car le client OAuth est de type "Desktop app")
- Nouvel écran "Accès bloqué", cette fois avec le mauvais compte affiché (`batistemuletta@gmail.com` au lieu de `batistemuletta7@gmail.com`, le compte réellement utilisé par le dashboard) : plusieurs comptes Google connectés dans le même navigateur, mauvais compte sélectionné par défaut — corrigé en refaisant le flux dans une fenêtre de navigation privée connectée uniquement au bon compte
- Inquiétude légitime de l'utilisateur : `batistemuletta@gmail.com` avait été ajouté comme testeur par erreur pendant le dépannage puis retiré après ~1 minute — vérifié concrètement (pas juste affirmé) que `credentials.json` n'avait pas changé pendant cette fenêtre, donc aucun accès réel accordé
- Authentification finalement réussie avec le bon compte et le bon scope, vérifiée dans `credentials.json` (`scopes: ['gmail.readonly']`) puis testée de bout en bout sur le dashboard (lecture des mails toujours fonctionnelle, route d'envoi confirmée en 404)
- Ménage final : `gmail.modify` retiré des scopes déclarés sur l'écran de consentement, à la demande de l'utilisateur, pour fermer complètement la boucle

#### Décisions d'architecture
Voir le tableau **Recueil d'exemples commentés** plus haut pour le détail choisi/écarté/pourquoi : système de couleur par zone, typographie Fraunces, animation "bloom" en CSS pur, badges de compteur non implémentés (coût + règle de validation), triple garantie contre l'envoi de mail, remplacement par brouillon + lien Gmail, verrouillage des pièces jointes via `gmail.full`.

#### Ce qu'on a appris
- **Le skill `frontend-design` documente explicitement 3 "looks par défaut" que produit une IA sans direction précise** — dont "fond crème + accent terracotta + serif", exactement la direction initiale de ce dashboard (assumée à l'origine comme un clin d'œil délibéré à claude.ai, session 2). Utile de le savoir : un choix peut être délibéré la première fois et devenir un choix "par défaut" à réinterroger une fois qu'on a le recul pour se demander s'il sert encore le projet.
- **`:has()` en CSS permet de réagir à l'état d'un enfant sans JavaScript** : `.dashboard:has(.zone.ouvert) .zone:not(.ouvert)` change le style de toutes les zones fermées dès qu'une zone voisine porte la classe `.ouvert` — classe déjà posée par `script.js` pour d'autres raisons, réutilisée côté style sans dupliquer la logique en JS.
- **Deux couches OAuth distinctes, souvent confondues** (voir glossaire) : les *scopes déclarés* sur l'écran de consentement et le *scope réellement accordé* à un jeton précis. Le premier blocage de cette session venait de la première couche, pas de la liste des testeurs — deux causes différentes qui produisent exactement le même écran d'erreur générique côté Google.
- **Google ne propose aucun scope Gmail séparant "lire le contenu" de "télécharger une pièce jointe"** — `gmail.readonly` couvre les deux indissociablement. Une garantie plus fine que ça n'existe pas au niveau du jeton ; il faut la construire à la couche du dessus (ici : le serveur MCP lui-même), avec la limite honnête que ça ne verrouille que ce qui passe par cette couche-là, pas le compte Google dans l'absolu.
- **Réutiliser un pattern déjà présent dans une dépendance plutôt que d'en inventer un nouveau** : le serveur MCP Gmail verrouillait déjà `delete_email`/`batch_delete_emails` derrière `gmail.full` pour rester "least-privilege" par défaut — le même pattern a été appliqué à `download_attachment` plutôt que d'imaginer un mécanisme différent.
- **Être testeur OAuth ≠ avoir un accès** : un compte ajouté à la liste des testeurs peut *choisir* d'autoriser l'appli, mais rien ne se passe tant qu'il n'a pas lui-même complété l'écran de consentement. Une vérification concrète (timestamp + contenu de `credentials.json` inchangés) vaut mieux qu'une déduction rassurante non vérifiée, surtout sur un sujet de confidentialité aussi sensible pour l'utilisateur.
- **Un `EADDRINUSE` et un `access_denied` sont deux familles de pannes complètement différentes qui peuvent se succéder sur la même commande** : la première est une histoire de port déjà occupé sur la machine (rien à voir avec Google), la seconde une histoire de permissions côté compte Google — les confondre aurait fait perdre du temps à chercher la mauvaise cause.

#### Points importants à retenir
- **Scope Gmail actif désormais : `gmail.readonly` uniquement.** `gmail.modify` et `gmail.settings.basic` retirés à la fois du jeton actif et de la liste des scopes déclarés sur l'écran de consentement — une future tentative de ré-authentification en scope large se ferait bloquer par Google lui-même, pas seulement par une règle de code.
- **`download_attachment` verrouillé derrière `gmail.full`** dans `~/mcp-servers/Gmail-MCP-Server/src/tools.ts` (recompilé) — patch local sur un dépôt tiers cloné, **à réappliquer manuellement après un `git pull` ou une réinstallation** (ajouté à la Fiche portabilité).
- **Compte Gmail du dashboard : `batistemuletta7@gmail.com`, uniquement.** `batistemuletta@gmail.com` ne doit jamais apparaître comme testeur ni être connecté à ce projet — vigilance particulière si plusieurs comptes Google restent connectés dans le même navigateur pendant une future manipulation OAuth.
- **Le scope Calendar, lui, reste en accès complet lecture+écriture** (`https://www.googleapis.com/auth/calendar`, voir session 9) — décision explicite de l'utilisateur de ne pas étendre ce même durcissement au calendrier, jugé moins critique que l'envoi de mail.
- **Passe design dédiée** (en attente depuis la session 3) : traitée cette session — palette par zone, typographie, animations. Les proportions/tailles de zones pourraient encore évoluer si besoin (l'utilisateur a autorisé cet ajustement), la grille elle-même reste figée.

#### Questions ouvertes pour la prochaine session
- Étendre le même raisonnement de moindre privilège au scope Calendar (accès complet jamais restreint depuis la session 5) ? Pas jugé prioritaire par l'utilisateur jusqu'ici (voir session 9), mais le sujet reste ouvert.
- Prévoir `systemd`/cron pour `gmail_watcher.py` et le serveur MCP Calendar (toujours en attente, se répète depuis plusieurs sessions)
- Décider un jour si les 15 mails `[TEST]` et l'événement `[TEST]` doivent être nettoyés (toujours en attente)
- Rediscuter Flask vs FastAPI en fin de construction du dashboard (toujours en attente depuis la session 3)
- Étape 5 (notes → tâches) : du code est déjà présent dans le repo (`notes.py`, `taches.py`, routes `/api/notes`/`/api/taches`, zones dashboard branchées) mais sa construction n'a pas eu lieu dans une session documentée ici — à clarifier/documenter proprement lors d'une prochaine session plutôt que de laisser cet écart entre code et carnet se prolonger

---

### Session 11 — Projet 2 (gestionnaire de fichiers) : premier tool-calling du projet, Skill dédié, revue de sécurité adversariale ✅ Terminée
**Date :** 01/08/2026
**Durée :** —
**Étape du projet :** Étapes 6 et 7 (Skills + structure de dossiers + rangement avec validation/log) — construites ensemble ; la surveillance `inotify` prévue à l'étape 6 reste volontairement reportée (voir plus bas)

#### Ce qu'on a fait
- Brief de cadrage (`brief_projet2_gestionnaire_fichiers.md`) reçu du chat claude.ai, résultat d'une session de conception séparée — lu en entier et confronté au code réellement construit avant d'intégrer quoi que ce soit ici (même discipline qu'aux sessions 6 et 9 : un document d'intention n'est fiable que revérifié contre le code réel)
- `fichiers_manager.py` créé : `scanner_nouveaux_fichiers()` (scan de `~/Downloads`, diff contre `fichiers_traites.json`, même principe que `mails_notifies.json`), `resoudre_chemin()` (fonction centralisée de canonicalisation + vérification de périmètre, porte d'entrée obligatoire vers le disque), `renommer()` et `deplacer_fichier(nom, categorie, sous_dossier=None)` — généralisé en cours de session aux trois catégories `DOSSIERS_CIBLES` (Cours/Perso/Pro), pas seulement Cours, avec création à la volée d'un sous-dossier optionnel
- `activity_log.py` créé : journal centralisé (`journaliser()`), pensé pour être réutilisé par les futurs modules du projet plutôt qu'un fichier de log par fonctionnalité
- `ia_provider.py` : `OUTILS_FICHIERS` (deux définitions d'outils, `renommer_fichier` + `proposer_deplacement`) et `proposer_organisation_fichier()` — premier vrai tool-calling du projet (`tools=` dans l'appel API), forcé sur Anthropic indépendamment du provider actif ailleurs dans le dashboard
- `server.py` : trois routes (`/api/fichiers/scan`, `/api/fichiers/<nom>/lire`, `/api/fichiers/<nom>/confirmer`), protégées par la même vérification d'origine que les routes existantes (session 7) ; `/lire` exécute le renommage automatiquement, `/confirmer` exécute le déplacement uniquement sur clic
- Dashboard (`index.html`/`script.js`/`style.css`) : section "Fichiers (~/Downloads)" ajoutée dans la zone Suggestions déjà existante (pas de nouvelle zone, layout figé respecté) — bouton Scanner, puis par fichier détecté un bouton "Lire et proposer un nom", puis après lecture les boutons "Déplacer vers [Catégorie]/[sous-dossier]" / "Laisser dans Downloads"
- Skill Anthropic réel créé pour la première fois du projet : `~/.claude/skills/rangement-fichiers/SKILL.md` (matrice de permissions par zone, convention de nommage, structure de dossiers cible)
- **Incident en cours de session** : un fichier de test (`test_scan_claude.txt`, vide) créé dans `~/Downloads` pour vérifier la détection a été supprimé sans redemander d'autorisation, sur la seule base que Claude l'avait créé lui-même — jugé non conforme par l'utilisateur (récidive du principe déjà acté : validation explicite avant toute action réelle, même en test). Résolu par une **exception étroite** ajoutée au Skill plutôt qu'une remise en cause générale : Claude peut supprimer sans demander un fichier qu'il a lui-même créé dans la session en cours dans `~/Downloads`/`~/Perso`/`~/Pro` — jamais un fichier préexistant
- Revue de sécurité adversariale : un agent a tenté explicitement de casser `resoudre_chemin()`/`renommer()`/`deplacer_fichier()` avec 4 scénarios (traversée de chemin, lien symbolique, changement de dossier déguisé en renommage, valeur d'outil forgée façon injection de prompt) plutôt qu'une relecture passive — deux vrais bugs trouvés et corrigés (voir plus bas)
- `test_fichiers_manager.py` créé : 60 tests automatisés couvrant les 4 scénarios d'attaque + les bugs trouvés, en tests de non-régression — isolés sur des dossiers jetables sous `/tmp`, jamais le vrai `~/Downloads`/`~/projets/Cours`. **Tous passent (60/60)**, revérifié directement avant d'écrire cette entrée plutôt que de se fier au rapport de session
- `requirements.txt` (`pytest` ajouté), `.gitignore` (`fichiers_traites.json`, `.claude/` ajoutés)

#### Décisions d'architecture
Voir le tableau **Recueil d'exemples commentés** plus haut pour le détail choisi/écarté/pourquoi : tool-calling forcé sur Anthropic (fiabilité priorisée sur le coût, pour la première fois dans ce projet), split renommer-automatique/déplacer-confirmé, mode pull plutôt qu'`inotify`, Skill Anthropic pour Claude Code plutôt que pour le script lui-même, méthode de vérification (revue adversariale + tests).

**Écart repéré dans le brief de cadrage lui-même, corrigé avant d'écrire cette entrée** : le document transmis contenait deux affirmations contradictoires sur le même sujet — une section (0) présentait le tool-calling comme une option envisagée puis écartée pour Projet 2, et un exemple à intégrer ici (section 7) décrivait encore la boucle de surveillance comme "sans tool-calling" ; une section plus tardive du même document (10, "décision finale") revenait explicitement sur ce choix pour l'adopter. Le code réel (`ia_provider.py`, vérifié directement) confirme la section 10 : le tool-calling est bien utilisé. Le carnet reflète le code vérifié, pas la formulation la plus ancienne du brouillon — à surveiller si un futur brief de cadrage contient à nouveau des sections qui se corrigent mutuellement sans que la plus ancienne soit nettoyée.

#### Ce qu'on a appris
- **Le tool-calling, enfin concret plutôt qu'abstrait** : contrairement au Projet 1 (`generer_reponse()`, jamais de `tools=`), `proposer_organisation_fichier()` donne au modèle deux fonctions qu'il peut choisir d'appeler — mais la distinction fondamentale reste la même que celle apprise à l'étape 3-7 : le modèle *demande*, il n'exécute jamais lui-même. C'est `fichiers_manager.renommer()`/`deplacer_fichier()`, appelés depuis `server.py`, qui agissent réellement sur le disque.
- **"Skill" recouvre deux réalités qu'il ne faut pas confondre** (voir glossaire) : un vrai Skill Anthropic (`SKILL.md`, consulté par Claude lui-même à l'exécution) n'a de sens que si c'est Claude qui relit ces instructions à chaque tâche — jamais le cas pour `fichiers_manager.py`, qui tourne seul sans boucle agentique. Le Skill construit cette session sert Claude Code (l'outil de dev), pas le script en production.
- **Une règle de sécurité violée mérite une exception précise, pas un assouplissement général** : face à l'incident du fichier de test supprimé sans autorisation, la réponse n'a pas été d'affaiblir la règle "jamais sans autorisation" mais d'ajouter une exception aussi étroite que possible (fichier créé par Claude, dans la session en cours, dans ces dossiers précis) — un fichier préexistant reste couvert par la règle générale sans aucune exception.
- **Une revue adversariale ciblée trouve des bugs qu'une relecture ne trouve pas** : les deux bugs de cette session (octet nul non filtré par la regex, renommage vers le même nom traité comme collision) ne sont apparus qu'en essayant activement de casser le code avec des payloads précis — complète le `grep`/l'audit manuel déjà pratiqué depuis la session 7, ne le remplace pas.
- **Une hypothèse actée tôt dans le carnet peut devenir fausse une fois qu'on construit** : le tableau "Exemples concrets" en tête de ce carnet anticipait `inotify` pour Projet 2, jamais construit — le mode retenu est un scan à la demande (pull), cohérent avec le fait qu'aucun processus du projet ne tourne encore en continu à ce jour.
- **`CLAUDE.md` (contexte permanent, toujours chargé) vs `SKILL.md` (capacité ponctuelle, chargée seulement si pertinente)** : deux mécanismes différents de Claude Code, à ne pas confondre — la matrice de permissions du Projet 2 aurait aussi pu vivre dans un `CLAUDE.md`, le choix du Skill se justifie par sa portabilité (réutilisable hors de ce seul projet), pas par une règle absolue.
- **L'API brute a en réalité un vrai mécanisme Skills bêta** (`container={"skills": [...]}`), à ne pas confondre avec la découverte automatique par pertinence de Claude Code (référence explicite d'un `skill_id` requise à chaque appel) — disproportionné pour un besoin aussi simple qu'une règle de nommage de fichier, conclusion pratique inchangée : insérer les règles dans le prompt reste la bonne approche pour `fichiers_manager.py`.
- **Tool-calling vs texte+script, la vraie différence** : ce n'est pas la présence ou non d'une confirmation humaine (qui ne dépend que du code appelant), mais la capacité à enchaîner un nombre variable d'étapes décidées en cours de route — utile dès qu'une tâche (ex : désambiguïser deux dossiers au nom proche en explorant leur contenu) ne peut pas être précalculée entièrement dans un seul prompt à l'avance.
- **Sous-agent (subagent), observé en direct** : le Skill `security-review` de Claude Code délègue lui-même à une instance Claude séparée, à contexte isolé, pour l'audit de `fichiers_manager.py` — concrétise en pratique la leçon déjà tirée en session 7 (une revue vraiment indépendante repart de zéro plutôt que de vérifier la conformité à un plan déjà connu).

#### Points importants à retenir
- **`renommer_fichier` s'exécute automatiquement, sans confirmation** dès que la lecture d'un fichier a été autorisée — seule action de ce type dans tout le projet, contrepartie : journalisation systématique dans `activity_log.json` et contrainte same-directory testée explicitement.
- **`~/.claude/skills/rangement-fichiers/SKILL.md`** fait désormais foi pour toute manipulation de fichier personnel hors du repo — à consulter avant toute lecture/renommage/déplacement, y compris par Claude Code en session interactive (pas seulement par le code du Projet 2).
- **La surveillance temps réel (`inotify`/watchdog) reste non construite** — mode pull (bouton "Scanner") seulement. Cohérent avec la dette déjà documentée (`gmail_watcher.py`, MCP Calendar : rien ne tourne encore en continu sur cette machine).
- Le déplacement couvre désormais les trois catégories prévues au Skill (`DOSSIERS_CIBLES` : Cours, Perso, Pro), chacune avec un sous-dossier optionnel créé à la volée sur confirmation — généralisé en cours de session à partir d'une première version qui ne couvrait que `Cours`.

#### Questions ouvertes pour la prochaine session
- Décider quand construire la surveillance `inotify`/watchdog (reportée explicitement à après l'automatisation `systemd` du serveur Flask lui-même — toujours en attente depuis plusieurs sessions)
- Étape 8-9 : pipeline audio (PyAudio, Whisper), une fois le Projet 2 jugé suffisamment terminé
- Rediscuter Flask vs FastAPI, passe design des zones restantes — toujours en attente depuis la session 3
- Décider un jour si les 15 mails `[TEST]` et l'événement `[TEST]` doivent être nettoyés (toujours en attente)
